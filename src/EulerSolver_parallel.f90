        ...
        do
         ...
         select case (nDim)
          ...
          case (2)
           ...
           if (lthreading) call omp_set_num_threads(num_threads)        ! Set max threads if enabled
           !$OMP parallel private(i,j,k,ProblemRegion) shared(ProblemRegion2D)
           !$OMP do
           ...
           if (lthreading) call omp_set_num_threads(num_threads)
           !$OMP parallel private(i,j,k,ProblemRegion) shared(ProblemRegion2D)
           !$OMP do
          ...
          case (3)
           ...
           if (lthreading) call omp_set_num_threads(num_threads)
           !$OMP parallel private(i,j,k,ProblemRegion) shared(ProblemRegion3D)
           !$OMP do
           ! For each strip in x direction, copy to 1D region, then solve
           do j = 1, ycells
            do k = 1, zcells
